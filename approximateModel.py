...
def main(train_file, logit_folder, savename, num_epochs=500, margin=25,
         base=0.01, mb_size=50):
    print("Loading data...")
    tr_addresses, num_tr = get_metadata(train_file)
    # VALIDATION DATA?
    ...
    im_shape = (227, 227)
    temp = 5.55     # 51 classes: 50 + other
    print("Building model and compiling functions...")
    network = build_cnn(im_shape, temp, input_var=input_var)
    # Losses and updates
    prediction = lasagne.layers.get_output(network)
    loss = -T.sum(target_var*T.log(prediction), axis=1)
    loss = loss.mean()
...
def data_and_logit_generator(addresses, logit_folder, im_shape, mb_size,
                             temp=1, preproc=False, shuffle=True):
    ...
    for batch in batches:
        ...
        for idx in batch:
            ...
            targets.append(np.mean(softerMax(logits, temp), axis=0)[np.newaxis,:])
        im = np.dstack(images)
        im = np.transpose(im, (2,1,0)).reshape(-1,3,im_shape[0],im_shape[1])
        output = np.vstack(targets).astype(np.float32)
...
if __name__ == '__main__':
    main('/home/daniel/Data/ImageNetTxt/transfer.txt',
         ...
         '/home/daniel/Data/Experiments/running_error.npz',
         num_epochs=50, margin=25, base=0.01, mb_size=50)
